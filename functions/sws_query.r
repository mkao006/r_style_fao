...
##'
##' @param area numeric or character vector with countries' ids
##' (numeric) or names (character).
##' @param item numeric or character vector with items' ids (numeric)
##' or names (character).
##' @param ele numeric or character vector with elements' ids
##' (numeric) or names (character).
##' @param year numeric vector with years to fetch.
##' @param symb optional character vector with symbols (flags) to
##' fetch data type of.
##' @param melted logical, TRUE by default. Should the result data be
##' returned in long format (instead of one column per every year two
##' qcolumns: year and corresponded value).
##' @param value.names logical, TRUE by default. Should
##' identificational vectors (area and item) be converted from numeric
##' ids to names.
##' @param stringsAsFactors logical. Should character identificational
##' vectors be converted to factors.
##' @param dbquery optional string with SQL-query to request from SWS.
##' @param class.path optional string with path to Oracle Java Data
##' Base Connectivity library. By default 'ojdbc14.jar' in working
##' directory.
##' @param user optional string with name of SWS DB user instead of
##' default user 'demo'.
##' @param pass optional string with password of SWS DB user instead
##' of default password 'demo'.
##'
...
into the working directory or specify full path with class.path argument.")
    ...
    if(!ping("lprdbwo1.fao.org"))
        stop("SWS DB accepts only internal connections. Please get a cable and find the nearest ethernet socket :)")
    ...
    ## In case user specifies only one ele and and one item we want
    ## function to return colname of value as name of ele instead of
    ## value.  ...

    if(!missing(year) & symb) flag <-
        str_c('SYMB_', formatC(year - year_shift, width=2, format='d',
                               flag = '0'), collapse = ', ')
    if(!missing(year))
        year <-
            str_c('NUM_',
                  formatC(year - year_shift, width=2, format='d', flag='0'),
              collapse =', ')
    ...
    if(!missing(year))
        whatsql <- str_c(whatsql, year, sep=', ')
    if(!missing(year) & symb)
        whatsql <- str_c(whatsql, flag, sep=', ')


    ## FROM
    fromsql <- dbmain
    if(value.names)
        fromsql <- str_c(str_c('FAOSTAT.',fromsql),
                         'FAOSTAT.AREA, FAOSTAT.ITEM', sep = ', ')

    ## WHERE
    wheresql <- list()
    if(!missing(area))
        wheresql[length(wheresql) + 1] <-
    ...
    if(value.names)
        wheresql[length(wheresql) + 1] <-
            str_c('AREA.AREA = ', dbmain, '.AREA and item.item = ',
                  dbmain, '.item')
    ...
    if(melted){
        ...
        valueswithoutsymb <-
            dboutput[, colnames(dboutput)[str_detect(colnames(dboutput),
                                                     perl('^(?!symb)'))]]
        ...
        valueswithoutsymb$year <-
            as.numeric(str_replace(valueswithoutsymb$year, '^num_', ''))
